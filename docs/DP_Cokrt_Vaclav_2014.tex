...
\begin{introduction}
	\uv{\textit{Graf je základním objektem teorie grafů. Jedná se o reprezentaci množiny objektů, u které chceme znázornit, 
...
\textit{\uv{REST (Representational State Transfer) – je architektura rozhraní, navržená pro distribuované prostředí. REST navrhnul a popsal v roce 2000 Roy 
...
\section{Gephi}
\uv{\textit{Gephi je platforma pro vizualizaci a prozkoumávání všech druhů sítí a
...
umožňují ruční, interaktivní editaci grafu - změnu velikosti uzlu, barvy, pozice, popisku, a tak podobně. V \uv{Laboratoři dat} je zase kromě importu a exportu také možné ručně přidávat a odebírat
...
\uv{\textit{Gephi Toolkit obsahuje základní moduly (Graf, Rozložení, Filtry, IO\ldots) ve formě standardní Java knihovny, kterou může jakýkoli projekt použít pro své potřeby.
...
Avšak na rozdíl od \uv{Statistik} a \uv{Roz\-lo\-že\-ní}, některé buildery jsou vytvářeny v závislosti na obsahu grafu za pomoci třídy \texttt{CategoryBuilder}.
...
\lstinputlisting[caption=Změna velikosti uzlu podle hodnotu atributu, firstline=31, lastline=41]{source_code_examples/RankingExample.java}
...
(česky jedináček nebo také unikát) je název pro návrhový vzor, používaný při programování. Využijeme ho při řešení problému, kdy je potřeba, aby v celém programu
 ...
 \uv{\textit{(\ldots) many modules just do their job on this current workspace and doesn't allow to specify which workspace to work on from the API. 
...
\lstinputlisting[caption=Import grafu do Workspace\, který nastavený jako \uv{current}, firstline=31, lastline=37, label=sourcecode:bug1]{source_code_examples/ConcurrencyBugExample.java}
...
\begin{description}
  ...
  \item[Potřebné oprávnění] Žádné
  \item[Tělo požadavku] Prázdné
  \item[Tělo odpovědi] Popis konkrétní funkce (jméno, parametry, \ldots) (ukázka \ref{rest:resp:statistic})
\end{description}
...
používaly \uv{current workspace} (problém je popsán v kapitole \ref{concurrency}).

To samozřejmě neřeší problém zcela, pokud se \uv{current workspace} používá např. uvnitř samotného pluginu, plugin nebude fungovat správně. To je však chyba 
...
Pohled lze přepnout na záložku \uv{Current graph}, kde jsou zobrazeny informace o aktuálním grafu včetně historie editace.

\subsubsection{Data aktuálního grafu}
Střední sloupec zobrazuje detail aktuálního grafu. Pro uživatele je nejdůležitější záložka \uv{SVG}, která zobrazuje vizuální podobu grafu. V záložce \uv{GEXF} je zobrazen 
...
\begin{figure}\centering
 	...
 	\caption[Aplikace funkce \uv{Hodnocení}]{Aplikace funkce \uv{Hodnocení} - obarvení a změna velikosti uzlu podle atributu \texttt{pagerank}}\label{fig:clientapp-06-ranking}
\end{figure}
\begin{figure}\centering
...
\begin{figure}
	\dirtree{%
		.1 webgephi.
			.2 webgephi-api\DTcomment{Knihovna s JAXB modelovými třídami RESTového rozhraní}.
				.3 src.
					.4 main.						
					.4 test.						
				.3 target.
					.4 webgephi-api-\{version\}.jar\DTcomment{Finální knihovna}.
			.2 webgephi-server\DTcomment{Hlavní aplikace \textit{WebGephi Server}}.
				.3 src.
					.4 main.						
					.4 test.						
				.3 target.
					.4 webgephi-server-\{version\}.war\DTcomment{Výsledný WAR archiv určený k nahrání na server}.
			.2 webgephi-client\DTcomment{Knihovna pro jednodušší volání REST rozhraní \textit{Serveru} (konektor)}.	
				.3 src.
					.4 main.						
					.4 test.						
				.3 target.
					.4 webgephi-client-\{version\}.jar\DTcomment{Finální knihovna}.	
			.2 webgephi-clientapp\DTcomment{Ukázková aplikace \textit{WebGephi Client App}}.
				.3 src.
					.4 main.						
					.4 test.						
				.3 target.
					.4 webgephi-clientapp-\{version\}.war\DTcomment{Výsledný WAR archiv určený k nahrání na server}.	
	}
	\caption[Struktura Maven modulů aplikace \textit{WebGephi}]{Struktura Maven modulů aplikace \textit{WebGephi}}\label{fig:webgephi-maven-structure}
\end{figure}

\begin{figure}\centering
 	\includegraphics[width=0.7\textwidth]{images/diagram/maven-dependencies}
 	\caption[Závislosti mezi \textit{WebGephi} moduly]{Závislosti mezi \textit{WebGephi} moduly}\label{fig:maven-dependencies}
\end{figure}

\section{Build}
Zdrojový kód aplikace je veřejně dostupný na \textit{GitHubu}\footnote{Umístění projektu: https://github.com/Beziks/webgephi}, případně na přiloženém CD.
Pro kompilaci projektu je nutné mít nainstalovaný nástroj Apache Maven.
...
\begin{description}
	...
	\item[OAUTH\_CONSUMER\_SECRET] \uv{Heslo} klientské aplikace, jak je nastaveno na \textit{WebGephi Serveru}. Např.:\\\texttt{-DOAUTH\_CON\-SU\-MER\_SECRET=}\texttt{19c6d6d6-e2ab-4b2d-b625-24925677bdc5}
\end{description}
...
utility\footnote{Přesný postup je popsán na stránkách \tetxit{Gephi} \url{https://wiki.gephi.org/index.php/How\_to\_use\_Gephi\_plug-ins\_with\_the\_Gephi\_Toolkit}.}.
...
\begin{figure}
	\dirtree{%
		...
		.1 application\DTcomment{aplikace připavená pro deploy na server}.
			.2 webgephi-server-1.0.war\DTcomment{serverová část aplikace}.
			.2 webgephi-clientapp-1.0.war\DTcomment{ukázková klientská aplikace}.
		.1 src.
			.2 webgephi.zip\DTcomment{zdrojové kódy aplikace}.
			.2 thesis.zip\DTcomment{zdrojová forma práce ve formátu \LaTeX{}}.
		.1 text\DTcomment{text práce}.
			.2 DP\_Cokrt\_Vaclav\_2014.pdf\DTcomment{text práce ve formátu PDF}.			
	}
\end{figure}
